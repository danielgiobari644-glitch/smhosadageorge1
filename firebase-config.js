...
const FALLBACK_DATA = {
  "settings/theme": {
    ...
    heroBtn1Text: 'Livestream',
    heroBtn1Link: '#livestream',
    heroBtn2Text: 'Give Online',
    heroBtn2Link: '#offering',
    joinFamilyTag: 'Our Community',
    joinFamilyTitle: 'Ready to Join the Family?',
    joinFamilySubtext: 'We believe everyone has a home, a purpose, and a family waiting for them here. Take your next steps to connect and grow with us.',
...
function dispatchLocalDataChange(path, collectionName) {
    try {
        if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('localstorage_data_changed', { detail: { path, collectionName } }));
        }
    } catch (e) {}
}
...
function wrapDocRef(originalDocRef, collectionName, docId) {
    ...
    return {
        ...
        set: async (data, options) => {
            ...
            if (!isFirestoreOffline && originalDocRef) {
                try {
                    await withTimeout(originalDocRef.set(data, options), 12000);
                } catch (err) {
        ...
        update: async (data) => {
            ...
            if (!isFirestoreOffline && originalDocRef) {
                try {
                    await withTimeout(originalDocRef.update(data), 12000);
                } catch (err) {
        ...
        delete: async () => {
            ...
            if (!isFirestoreOffline && originalDocRef) {
                try {
                    await withTimeout(originalDocRef.delete(), 12000);
                } catch (err) {
...
function wrapCollectionRef(originalCollRef, collectionName) {
    return {
        ...
        get: async (options) => {
            ...
            try {
                if (originalCollRef) {
                    const snap = await withTimeout(originalCollRef.get(options), 10000);
                    const listToCache = [];
                    snap.forEach(doc => {
                        listToCache.push({ id: doc.id, ...doc.data() });
                    });
...
function wrapQuery(originalQuery, collectionName, queryOp) {
    return {
        ...
        get: async (options) => {
            ...
            try {
                if (originalQuery) {
                    const snap = await withTimeout(originalQuery.get(options), 10000);
                    return snap;
                }
