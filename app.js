    ...
    try {
        ...
        if (doc && doc.exists) {
            ...
            if (theme.primaryColor) {
                ...
                const hoverColor = theme.primaryHover || darkenColor(theme.primaryColor, 12);
                if (hoverColor) {
            ...
            if (theme.sectionSpacing) {
                document.documentElement.style.setProperty('--section-spacing', theme.sectionSpacing + 'rem');
            }
            if (theme.fontSizeBase) {
                document.documentElement.style.setProperty('--font-size-base', theme.fontSizeBase + 'px');
            }
            ...
            if (theme.primaryColor) {
                ...
                if (!theme.primaryHover) {
                    const hoverColor = darkenColor(theme.primaryColor, 12);
                    document.documentElement.style.setProperty('--primary-hover', hoverColor);
                }
            }
            if (theme.primaryHover) {
...
async function loadQuotes() {
    try {
        ...
        if (validDocs.length > 0) {
            ...
            container.innerHTML = `
                <div class="quote-slide">
                    <blockquote>"The word of God is a lamp unto my feet and a light unto my path."</blockquote>
...
// ========================================
// Image Loading & Pop-In Animation Setup
// ========================================

function handleImagePop(img) {
    if (!img || img.dataset.popInited) return;
    img.dataset.popInited = 'true';
...
function initImagePopAnimations(root = document) {
...
if (typeof window !== 'undefined') {
    window.addEventListener('localstorage_data_changed', reloadAllAppData);
    window.addEventListener('storage', reloadAllAppData);
}
